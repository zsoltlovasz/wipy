...
class WS2812:
    ...
    #changing 1 to 1.0 at intensity, and renaming it to intensity_divider, because micropython doesn't accept float numbers
    def __init__(self, spi_bus=1, led_count=1, intensity_divider=1):
        """
        Params:
        * spi_bus = SPI bus ID (1 or 2)
        * led_count = count of LEDs
        * intensity_divider = light intensity (int >=1)
        """
        self.led_count = led_count
        self.intensity_divider = intensity_divider
    ...
    def update_buf(self, data, start=0):
        ...
        intensity_divider = self.intensity_divider

        mask = 0x03
        index = start * 12
        for red, green, blue in data:
            red = self.int_divide(red, intensity_divider)
            green = self.int_divide(green, intensity_divider)
            blue = self.int_divide(blue, intensity_divider)
